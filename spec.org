...
** code:
   - compute PWK:
     ...
     - PWK|MAC|accountID|SRPpw = HKDF(SKM=C, XTS=KW("four-keys"), CTXinfo="",
                                      L=4*dkLen)
   ...
   - initial key-wrapping setup (done once, during account setup):
     ...
     - SRPv = SRP(P=SRPpw, username=ID_b64)
       - use b64 encoded accountID, not raw accountID, to play nice with SRP
         libraries that expect email to be ASCII
     - send (accountID, SRPv) safely to server XXX
     - store initial WUK:
       - client:
         - WUK: WUKout_b64 = encrypt_and_mac(PWK, MAC, data=UK)
         - SRPKsession_b64,sessid = perform_SRP(accountID_b64, SRPpw_b64, SRPsalt="")
           - server remembers sessid->(Ksession,accountID) for a little while
         - make_session_keys()
         - send_request(["set", WUKout_b64])
         - response should be ["ok"]
       - server process_request(accountID, request):
         - if request[0] == "set": db[accountID]=request[1], return ["ok"]
   - retrieve WUK (from password)
     ...
     - server process_request(accountID, request):
       - if request[0] == "read": return ["ok", db[accountID]]
